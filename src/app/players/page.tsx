...
export default async function PlayersPage() {
  ...
  return <div className="page players-page">
    ...
    <section className="section-block person-directory"><div className="section-heading"><div><h2>排行榜</h2></div></div><div className="person-directory-list">{people.map(({ person, estimatedRank, summary, totalCompetitionPoints }) => <article key={person.id} style={{ "--player-color": person.color } as React.CSSProperties}><div className="person-directory-name"><PersonAvatar person={person} size="small" /><div><strong>{person.displayName}</strong><small>{person.kind === "human" ? "人类" : "AI"} · {person.aliases.slice(0, 3).join(" / ")}</small></div></div><div className={styles.directoryMetrics}><span>总 PT<strong className={totalCompetitionPoints >= 0 ? "positive" : "negative"}>{totalCompetitionPoints >= 0 ? "+" : ""}{totalCompetitionPoints.toFixed(1)}</strong></span><span>半庄<strong>{summary["对局数"] ?? 0}</strong></span><span>平均顺位<strong>{summary["平均顺位"]?.toFixed(2) ?? "-"}</strong></span><span>推定段位<strong>{formatEstimatedRank(estimatedRank)}</strong></span></div><Link className="icon-link" href={`/players/${encodeURIComponent(person.id)}`} title="查看人物数据" aria-label={`查看${person.displayName}数据`}><ArrowRight /></Link></article>)}</div></section>
  </div>;
}
