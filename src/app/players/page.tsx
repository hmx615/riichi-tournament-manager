...
export default async function PlayersPage() {
  ...
  return <div className="page players-page">
    ...
    <section className="section-block person-directory"><div className="section-heading"><div><h2>人物目录</h2></div></div><div className="person-directory-list">{people.map(({ person, summary, competitions }) => <article key={person.id} style={{ "--player-color": person.color } as React.CSSProperties}><div className="person-directory-name"><i /><div><strong>{person.displayName}</strong><small>{person.kind === "human" ? "人类" : "AI"} · {person.aliases.slice(0, 3).join(" / ")}</small></div></div><div><span>半庄<strong>{summary["对局数"] ?? 0}</strong></span><span>平均顺位<strong>{summary["平均顺位"]?.toFixed(2) ?? "-"}</strong></span><span>参赛<strong>{competitions.length}</strong></span></div><Link className="icon-link" href={`/players/${person.id}`} title="人物数据" aria-label={`查看${person.displayName}数据`}><ArrowRight /></Link></article>)}</div></section>
  </div>;
}
