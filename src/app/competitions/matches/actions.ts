...
/**
 * 通用牌谱录入服务：四席位对局赛与多阶段个人赛共用同一套解析、身份匹配与保存逻辑。
 * 个人赛通过 scheduleId 传入本桌赛程，从而限定参赛者范围。
 */
...
export async function parseMatchAction(_state: MatchEntryState, formData: FormData): Promise<MatchEntryState> {
  ...
  try {
    ...
    const deduped = preview.seats.filter((seat) => seat.resolvedByTable).length;
    const preferred = preview.seats.filter((seat) => seat.resolvedByPreference).length;
    return {
      status: "success",
      message: unresolved
        ? `解析成功，${unresolved} 个座次需要手动确认`
        : deduped || preferred
          ? `解析成功，四家身份已自动匹配（${[
            deduped ? `${deduped} 个共用账号按同桌去重确定` : "",
            preferred ? `${preferred} 个共用账号按偏好规则匹配` : "",
          ].filter(Boolean).join("，")}，请在确认页核对）`
          : "解析成功，四家身份已自动匹配",
      preview,
      operation: "create",
