...
export function IndividualCompetitionOverview({ competition, summary, showBackLink = false, admin }: {
  ...
  return <div className="page competition-page">
    ...
    <section className="standings">
      ...
      <div className="table-wrap"><table className="match-table individual-standings-table"><thead><tr><th>排名</th><th>选手</th><th>积分</th><th>与上一名差</th><th>已打半庄</th><th>平均顺位</th><th>状态</th></tr></thead><tbody>{sortedPlayers.map((participant, index) => { const status = currentStage(participant.id); const statusClass = status === "冠军" ? "stage-final champion" : status === "亚军" ? "stage-final runner-up" : status === "季军" ? "stage-final third-place" : status === "决赛" ? "stage-final" : status === "半决赛" ? "stage-semifinal" : status === "初赛" ? "stage-preliminary" : "stage-eliminated"; const previous = index > 0 ? totals[sortedPlayers[index - 1].id] ?? 0 : null; const gap = previous == null ? null : previous - (totals[participant.id] ?? 0); const line = !activeStageComplete && stageAdvancingCount > 0 && index === stageAdvancingCount; return <React.Fragment key={participant.id}>{line && <tr className="qualification-line"><td colSpan={7}>晋级线</td></tr>}<tr className={status === "已淘汰" ? "eliminated-standing-row" : ""} key={participant.id}><td><strong>{index + 1}</strong></td><td><PlayerTag participant={participant} /></td><td className={(totals[participant.id] ?? 0) >= 0 ? "positive" : "negative"}>{(totals[participant.id] ?? 0) >= 0 ? "+" : ""}{(totals[participant.id] ?? 0).toFixed(1)}</td><td>{gap == null ? "-" : gap.toFixed(1)}</td><td>{summary[participant.id]?.["对局数"] ?? 0}</td><td>{summary[participant.id]?.["平均顺位"]?.toFixed(2) ?? "-"}</td><td><span className={`stage-status ${statusClass}`}>{status}</span></td></tr></React.Fragment>; })}</tbody></table></div>
    </section>
    <section className="section-block">
      <div className="section-heading"><div><h2>赛程</h2></div><span className="table-count">{completed} 场已完成</span></div>
      {settings && <ul className="workflow"><li className={`workflow-stage workflow-${stageStatus("preliminary") === "进行中" ? "active" : stageStatus("preliminary") === "已完成" ? "done" : "pending"}`}><b>1</b><div><strong>初赛 <em className="stage-seal">{stageStatus("preliminary")}</em></strong><small>{settings.stages.preliminary.matchCountPerPlayer} 半庄/人{(settings.stages.preliminary.advancingPlayerCount ?? 0) > 0 ? ` · ${settings.stages.preliminary.advancingPlayerCount} 人晋级半决赛` : ""}{(settings.preliminaryDirectFinalPlayerCount ?? 0) > 0 ? ` · ${settings.preliminaryDirectFinalPlayerCount} 人直通决赛` : ""}</small></div></li><li className={`workflow-stage workflow-${stageStatus("semifinal") === "进行中" ? "active" : stageStatus("semifinal") === "已完成" ? "done" : "pending"}`}><b>2</b><div><strong>半决赛 <em className="stage-seal">{stageStatus("semifinal")}</em></strong><small>{settings.stages.semifinal.matchCountPerPlayer} 半庄/人{(settings.semifinalAdvancingPlayerCount ?? settings.stages.semifinal.advancingPlayerCount ?? 0) > 0 ? ` · ${settings.semifinalAdvancingPlayerCount ?? settings.stages.semifinal.advancingPlayerCount} 人晋级决赛` : ""}</small></div></li><li className={`workflow-stage workflow-${stageStatus("final") === "进行中" ? "active" : stageStatus("final") === "已完成" ? "done" : "pending"}`}><b>3</b><div><strong>决赛 <em className="stage-seal">{stageStatus("final")}</em></strong><small>{settings.stages.final.matchCountPerPlayer} 半庄/人</small></div></li></ul>}
      <h3>待开赛</h3>
