...
export function IndividualCompetitionOverview({ competition, summary, showBackLink = false, admin }: {
  ...
  return <div className="page competition-page">
    ...
    <section className="section-block">
      ...
      {settings && <ul className="workflow"><li><b>1</b><div><strong>初赛</strong><small>{settings.stages.preliminary.matchCountPerPlayer} 半庄/人 · {settings.stages.preliminary.advancingPlayerCount ?? 0} 人晋级半决赛 · {settings.preliminaryDirectFinalPlayerCount ?? 0} 人直通决赛</small></div></li><li><b>2</b><div><strong>半决赛</strong><small>{settings.stages.semifinal.matchCountPerPlayer} 半庄/人 · {settings.semifinalAdvancingPlayerCount ?? settings.stages.semifinal.advancingPlayerCount ?? 0} 人晋级决赛</small></div></li><li><b>3</b><div><strong>决赛</strong><small>{settings.stages.final.matchCountPerPlayer} 半庄/人</small></div></li></ul>}
...
export function IndividualCompetitionData({ competition, summary }: { competition: Competition; summary: CompetitionSummary }) {
  ...
  return <section className="section-block">
    ...
    <div className="table-wrap"><table className="match-table"><thead><tr><th>排名</th><th>选手</th><th>累计积分</th><th>半庄数</th><th>平均顺位</th><th>顺位分布</th><th>立直多面率</th><th>立直好型率</th></tr></thead><tbody>{sortedPlayers.map((participant, index) => { const rankCounts = [1, 2, 3, 4].map((rank) => competition.matches.reduce((count, match) => count + (match.seats.find((seat) => seat.participantId === participant.id)?.rank === rank ? 1 : 0), 0)); return <tr key={participant.id}><td><strong>{index + 1}</strong></td><td><PlayerTag participant={participant} /></td><td className={(totals[participant.id] ?? 0) >= 0 ? "positive" : "negative"}>{(totals[participant.id] ?? 0) >= 0 ? "+" : ""}{(totals[participant.id] ?? 0).toFixed(1)}</td><td>{summary[participant.id]?.["对局数"] ?? 0}</td><td>{summary[participant.id]?.["平均顺位"]?.toFixed(2) ?? "-"}</td><td>{rankCounts.join(" / ")}</td><td>{rateLabel(summary[participant.id]?.["立直多面率"])}</td><td>{rateLabel(summary[participant.id]?.["立直好型率"])}</td></tr>; })}</tbody></table></div>
  </section>;
}
